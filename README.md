...
### v3.6 (2026-09-16) - Navigation Fix + Alert Lifecycle

**Fixed:**
- Navigasi Alert Center → section lain flash dashboard sekejap
- Klik "Perangkat"/"Absensi" dari `/alerts` redirect ke dashboard dulu
- Alert severity tidak berubah saat nilai naik/turun (warning → danger)
- Alert baru muncul alih-alih update severity di baris yang sama
- Timezone parsing di frontend (WIB UTC+7)
- `stopAttendancePolling is not defined` error
- Health ring tidak exclude device dengan alert
- Marker cluster tidak muncul di map full
- Chart drag bisa keluar grid boundary
- Delete device tanpa konfirmasi nama

**Added:**
- Alert lifecycle state machine (healthy → warning → danger → clear)
- Hysteresis untuk mencegah alert flicker
- Global JS error boundary
- Back navigation pintar (kembali ke halaman asal)
- Regenerate API key dari device list
- Markdown sanitization di semua render
- Anti-flash section (langsung ke section tanpa dashboard dulu)
- Migrasi schema v6 dengan index tambahan
- `alerts/trend` endpoint untuk grafik trend
- `dashboard` optimized (window function, no N+1)
- CORS wildcard reject
- Debug logging lebih informatif

**Changed:**
- Navigation pakai `<a href>` native (bukan `<div>` + JS)
- `_evaluate_severity` cek dari sempit ke luas
- Section di `content-area` default `display:none` via CSS
- Inline script anti-flash di `dashboard.html`
- Config `ALERT_RULES` dengan `_hysteresis` per sensor
