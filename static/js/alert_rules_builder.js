/* ==========================================
   ...
   Form visual untuk set custom alert tanpa JSON
   ========================================== */
...
(function() {
    ...
    function renderSeverityRow(severity, values) {
        return `
            <div class="severity-row ${severity}">
                ...
                <input type="number" step="0.1" class="severity-input"
                       data-severity="${severity}" data-field="min"
                       placeholder="Min" value="${values.min}">
                <input type="number" step="0.1" class="severity-input"
                       data-severity="${severity}" data-field="max"
                       placeholder="Max" value="${values.max}">
            </div>
        `;
    }

    function renderRuleItem(sensorKey, rule) {
        const preset = window.SENSOR_PRESETS[sensorKey];
        ...
        let severityRows = '';
        if (rule.healthy) severityRows += renderSeverityRow('healthy', rule.healthy);
        if (rule.warning) severityRows += renderSeverityRow('warning', rule.warning);
        if (rule.danger) severityRows += renderSeverityRow('danger', rule.danger);

        item.innerHTML = `
            <div class="alert-rule-header">
                <select class="alert-rule-sensor-select">${options}</select>
                <button type="button" class="alert-rule-remove" title="Hapus">
                    <i class="fa-solid fa-trash"></i>
                </button>
            </div>
            <div class="alert-rule-severities">${severityRows}</div>
        `;

        // Attach listeners
        item.querySelector('.alert-rule-sensor-select').addEventListener('change', function() {
            onSensorChange(this);
        });
        item.querySelector('.alert-rule-remove').addEventListener('click', function() {
            onRemove(this);
        });
    ...
    function onSensorChange(selectEl) {
        ...
        item.querySelectorAll('.severity-row').forEach(row => {
            const severity = Array.from(row.classList).find(c =>
                ['healthy', 'warning', 'danger'].includes(c)
            );
    ...
    window.addAlertRule = function(mode = 'add') {
        const container = document.getElementById(
            mode === 'add' ? 'addAlertRulesBuilder' : 'editAlertRulesBuilder'
        );
        if (!container) return;
        ...
        // Cari sensor yang belum dipakai
        const usedSensors = Array.from(container.querySelectorAll('.alert-rule-sensor-select'))
            .map(sel => sel.value);
        const availableSensor = Object.keys(window.SENSOR_PRESETS).find(s => !usedSensors.includes(s))
            || Object.keys(window.SENSOR_PRESETS)[0];
        ...
        container.querySelectorAll('.alert-rule-item').forEach(item => {
            ...
            item.querySelectorAll('.severity-row').forEach(row => {
                ...
                if (!isNaN(min) && !isNaN(max)) {
                    rule[severity] = { min, max };
                }
            });

            if (Object.keys(rule).length > 0) {
                rules[sensorKey] = rule;
            }
        });
    ...
    window.updateJsonPreview = function(mode) {
        ...
        const textarea = document.getElementById(
            mode === 'add' ? 'addDeviceAlertRules' : 'editDeviceAlertRules'
        );
        if (textarea) {
            textarea.value = rules ? JSON.stringify(rules, null, 2) : '';
        }
    };
})();