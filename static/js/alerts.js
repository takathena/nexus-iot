...
(function() {
    ...
    async function loadStats() {
        try {
            ...
            console.error('[Alerts] Failed to load stats:', e);
        }
    }
        ...
        state.filtered.forEach(a => {
            ...
            if (group !== currentGroup) {
                ...
                html += `<div class="alert-group">
                    <div class="alert-group-label">${group}</div>`;
                currentGroup = group;
                groupOpened = true;
            }
                ...
                <div class="alert-item-content">
                    ...
                    <div class="alert-item-meta">
                        <span>
                            ...
                            <span class="device-ref">${escapeHtml(a.device_name || a.device_id)}</span>
                        </span>
                        ${a.location ? `<span><i class="fa-solid fa-location-dot"></i> ${escapeHtml(a.location)}</span>` : ''}
                        <span title="${formatFullTime(a.created_at)}">
                            <i class="fa-regular fa-clock"></i> ${formatTime(a.created_at)}
                        </span>
                    </div>
                </div>
        ...
        document.querySelectorAll('[data-action="acknowledge"]').forEach(btn => {
            btn.addEventListener('click', async e => {
                ...
                if (ok) {
                    ...
                    window.showToast('Gagal menandai alert', 'error');
                }
            });
        });
